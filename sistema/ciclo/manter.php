    ...
    case 'temp':      
        if (!empty($cod_tipo_documento)) {
            ...
            if (pg_num_rows($q1) > 0) {                             
                ...
                fclose($handle); 
                $txt_caminho = getcwd()."/digital/".$txt_nome_arquivo;  
                copy($txt_caminho_completo, $txt_caminho);
                unlink($txt_caminho_completo);                              
                
                $retorno['url'] = $_SESSION["txt_caminho_aplicacao"]."/visualizador/web/viewer.html?file=".urlencode($_SESSION["txt_caminho_aplicacao"]."/ciclo/digital/".$txt_nome_arquivo);
