...
verifica_seguranca();
cabecalho();
permissao_acesso_pagina(1);

if (empty($_REQUEST['log'])) {
	Auditoria(46, "Listar Eixos", "");
}
 ...
 <div id="main" class="container-fluid" style="margin-top: 50px">
	<form id="frm1">  
		...
		<div id="top" class="row">
			...
			<div class="col-sm-3">
				<?php if (permissao_acesso(2)) { ?>
					<a href="incluir.php" class="btn btn-primary pull-right h2">Novo Eixo</a>
				<?php } ?>							
			</div>
		</div> <!-- /#top -->
		<hr />
		<div id="list" class="row">
			<?php
			try {
				$condicao = " WHERE LOWER(txt_eixo) LIKE '%" .trim(strtolower($txt_pesquisa)). "%' "; 
				$condicao .= " OR LOWER(codigo_eixo) LIKE '%" .trim(strtolower($txt_pesquisa)). "%'";
			}	
			catch(Exception $e) {	
				$condicao = "";					
			}
			$q1 = pg_query("SELECT * FROM tb_eixo " .$condicao. " ORDER BY txt_eixo");
			if (pg_num_rows($q1) > 0) {
			?>
				<div class="table-responsive col-md-12">
					<table id="dtBasicExample" class="table table-striped" cellspacing="0" cellpadding="0">
						...
						<tbody>
							...
							?>
								<tr>
									<td><?php echo($rs1['codigo_eixo']) ?></td>
									...
									<td class="actions">									
										<?php if (permissao_acesso(3)) { ?>
											<a class="btn btn-warning btn-xs" href="incluir.php?id=<?php echo($rs1['cod_eixo']) ?>">Editar</a>
										<?php } ?>
										<?php if (permissao_acesso(4)) { ?>
											<a class="btn btn-danger btn-xs" onclick="return Excluir(<?php echo($rs1['cod_eixo']) ?>);" >Excluir</a>
										<?php } ?>
									</td>
								</tr>		
							<?php
